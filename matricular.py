...
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "") # Deixe vazio, o Canvas preencherá em tempo de execução.
...
if not MP_ACCESS_TOKEN:
    _log("ERRO: MP_ACCESS_TOKEN não configurado. A integração com Mercado Pago não funcionará.")
else:
    mercadopago.config({
...
async def endpoint_iniciar_matricula(body: dict):
    ...
    curso_principal_nome = cursos_nomes[0] if cursos_nomes else "Matrícula CED"
    ...
    if not THANK_YOU_PAGE_URL:
        raise HTTPException(500, detail="THANK_YOU_PAGE_URL não configurada no ambiente.")
